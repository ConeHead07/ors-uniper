...
if (!isset($confName)) die("\$confName Is Undefined!<br>\n");
...
if (isset($confName) && isset($ConfRegData[$confName]) && file_exists($InclBaseDir.$ConfRegData[$confName])) {
	...
	$inputItem = new ItemEdit($_CONF[$confName], $connid, $user, $dataId);
	...
	if (!empty($editCmd) || $dataId) {
		...
		if (!empty($editCmd) && is_array($editCmd)) $editCmd = key($editCmd);
		// $body_content.= "#".__LINE__." editCmd:".print_r($editCmd, true)." <br>\n";
		if ($view == "del") $editCmd = "Drop";
		if (!isset($editCmd)) $editCmd = "Edit";
		...
		if ($inputItem->autorun_status == 0) {
			...
			switch($inputItem->autorun_status) {
				case 1:
				$body_content.= "<L".__LINE__."/> Datensatz gel�scht!<br>\n";
				break;
				
				case -1:
				$body_content.= "<L".__LINE__."/> Interner Fehler beim L�schen!<br>\n";
				break;
	...
	if ($show_list) {
		if ($userHasInsertAccess) {
			$body_content.= "[<a href=\"".$modBaseLink."&confName=".rawurlencode($confName)."&editCmd=Edit\">Neuen Eintrag hinzuf�gen</a>]<br>\n";
		} else {
