...
function check_integer($inputZahl, $min = false, $max = false) {
	...
	if (strval(intval($inputZahl)) === strval($inputZahl)) {
		if (!($min === false)) {
            if ($inputZahl < $min) {
                $auswertung["Error"].="Geben Sie eine Ganzzahl größer gleich $min ein!<br>";
            }
        }
		if (!($max === false)) {
            if ($inputZahl > $max) {
...
function check_float($inputZahl,$min,$max,$nachkommastellen) {
	...
	if (strval(doubleval($inputZahl)) === strval($inputZahl)) {
		if (!($min === false)) {
            if ($inputZahl < $min) {
                $auswertung["Error"].="Geben Sie eine Zahl größer gleich $min ein!<br>";
            }
        }
		if (!($max === false)) {
            if ($inputZahl > $max) {
   	 	...
   	 	$auswertung["Error"].="Geben Sie eine Zahl aus reinen Zahlen mit einem Punkt für Nachkommastellen an!<br>";
	 }
