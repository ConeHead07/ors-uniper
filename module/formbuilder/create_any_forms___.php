...
if (!is_resource($db_conn)) {
	die("Ung�ltige Zugangsdaten f�r DB-Server!<br>\n");
}
...
if (!isset($_POST["resetSrc"]) && !empty($aCnfForm["Database"])) {
	...
	if (isset($_POST["resetSrc"]) || !empty($aCnfForm["Table"])) {
		...
		$dbAccessForm.= getFormSelectBySQL("SetTbl", "SHOW Tables FROM `".$aCnfForm["Database"]."`", $db_conn, false, false, "", "Tbl ausw�hlen")."<br>\n";
	}
} else {
	$dbAccessForm.= "Datenbank: ".($db_only? "(<strong>DbOnly:</strong>".$db_only.")" : "")."<br>\n";
	$dbAccessForm.= getFormSelectBySQL("SetDb", "SHOW DATABASES", $db_conn, false, false, $default_db, "Db ausw�hlen")."<br>\n";
}

$dbAccessForm.= "Oberverzeichnis f�r $dirLib und $dirTpl ".($dir_only? "(<strong>DirOnly:</strong>".$dir_only.")" : "").":<br>\n";
...
if (!isset($_POST["resetSrc"]) && $aCnfForm["Database"] && $aCnfForm["Table"] && $aCnfForm["DirName"] && $aCnfForm["ConfName"]) {
	...
	// DB-Basisdaten f�r Conf-Erstellung
