...
if (function_exists("db_auto_connect")) {
    db_auto_connect();
}
...
if (empty($redirect)) {
    if (!empty($_POST['redirect'])   && trim($_POST['redirect']) ) {
        $redirect = $_POST['redirect'];
    }
    elseif (!empty($_GET['redirect']) && trim($_GET['redirect']) ) {
        $redirect = $_GET['redirect'];
    }
    else {
        $redirect = $_CONF['redirectAfterLogin'];
    }
}
	...
	if (isset($user['uid'])) {
		...
		if ($user['freigegeben'] == 'Ja') {
	        ...
	        if (function_exists("session_start")) {
				...
				if (strpos($redirect,'?') === false || strpos($redirect, '?area=start') !== false) {
                    switch($user['gruppe']) {
                        ...
                        case 'umzugsteam':
                            ...
                            $redirect = './?s=auslieferung';
                            break;

                        case 'user':
                        default:
                        $redirect = './index.php?s=Umzug';
                            break;
                    }
                }
