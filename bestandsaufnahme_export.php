...
$aGebaeudeFilter = array('XXX');

$srcDB_Request = "";
$srcDB = $MConf["DB_Name"]; //getRequest("srcDB", "");
...
if (!$srcDB) {
	echo "Es wurde ".(!$srcDB_Request?"keine Datenbank":"eine ung�ltige Datenbank $srcDB_Request")." angegeben!<br>\n";
}
...
if ($srcDB) {
	...
	for($i = 0; $i < count($aGebaeude); $i++) {
		...
		$aGebaeudeStat[$g] = array("Stadt"=>$aGebaeude[$i]["stadt"], "Etagen"=>"", "R�ume"=>"", "Alle Mitarbeiter"=>"", "Neue Mitarbeiter"=>"", "Gel�schte R�ume"=>"", "Gel�schte Mitarbeiter"=>"", "Adresse"=>$aGebaeude[$i]["adresse"]);
	}
	...
	for($i = 0; $i < count($rows); $i++) {
		...
		$aGebaeudeStat[$g]["R�ume"] = $e["raeume"];
	}
	
	$sql = "SELECT gebaeude, count(raumnr) raeume_geloescht \n";
	$sql.= "FROM `$srcDB`.`mm_stamm_immobilien_geloescht` \n";
	$sql.= "WHERE gebaeude IN(\"".implode("\",\"", $aGebKuerzel)."\")\n";
	...
	for($i = 0; $i < count($rows); $i++) {
		...
		$e = $rows[$i];
		$g = $e["gebaeude"];
		$aGebaeudeStat[$g]["Gel�schte R�ume"] = $e["raeume_geloescht"];
	}
	...
	for($i = 0; $i < count($aGebaeude); $i++) {
		...
		$g = $e["gebaeude"];
		/*
		$aGebaeudeStat[$g] = array("Etagen"=>"", "R�ume"=>"", "Alle Mitarbeiter"=>"", "Neue Mitarbeiter"=>"", "Gel�schte R�ume"=>"", "Gel�schte Mitarbeiter"=>"");
		...
		*/
		...
		if ($db->error()) echo $db->error()."\n".$sql."<br>\n";
		//else echo "row: ".print_r($row, 1)."<br>\n";
		$aGebaeudeStat[$g]["Gel�schte Mitarbeiter"]  = $row["mitarbeiter_geloescht"];
	}
}
...
echo "<table class=\"tblList\"><thead><tr><td>#</td><td>Geb�ude</td>";
if (!empty($aGebaeudeStat)) {
	...
	echo "<div style=\"font-size:11px;font-family:Arial,sans-serif;\">(Geb�ude, R�ume, Mitarbeiter, Bereiche, Abteilungen, Gel�schte R�ume u. Mitarbeiter)</div>\n";
}
echo "</form>\n";
...
if ($srcDB && $runExport) {
	...
	foreach($aExportFiles as $key => $aV) echo "\n<li><a href=\"".$aV["file"]."\">$key ".$aV["rows"].": ".$aV["file"]."</a></li>\n";
}
// Start-Import
...
if ($dstDB && $runImport) {
			...
			if (count($aValues) == count($aFields)) {
				...
				if (empty($row["id"])) {
					...
					if ($db->error()) echo "#".__LINE__." DB-ERR: ".$db->error()."\n".$sql."<br>\n";
						...
						$num_import++;
						echo "Neue Abteilung ".$e["abteilung"]." mit ".$e["anzahl"]." MA wurde importiert!<br>\n";
					}
				} elseif ($row["bereich"] != $e["bereich"]) {
					echo "Abteilung ".$e["abteilung"]." (Bereich ".$e["bereich"].") existiert bereits unter anderer Hauptabteilung ".$row["bereich"]."!<br>\n";
				}
			}
		}
	}
	echo "Es wurden $num_import neue Abteilungen importiert!<br>\n";
	fclose($fp);
			...
			if (count($aValues) == count($aFields)) {
				...
				if (empty($row["id"])) {
					...
					if ($db->error()) echo "#".__LINE__." DB-ERR: ".$db->error()."<br>\n";
						...
						$num_import++;
						echo "Neuer Bereich ".$e["bereich"]." mit ".$e["anzahl"]." MA wurde importiert!<br>\n";
					}
				} elseif ($row["organisationseinheit"] != $e["organisationseinheit"]) {
					echo "Bereich ".$e["bereich"]." (GF ".$e["organisationseinheit"].") existiert bereits unter anderem GF ".$row["organisationseinheit"]."!<br>\n";
				}
			}
		}
	}
	echo "Es wurden $num_import neue Bereiche importiert!<br>\n";
	fclose($fp);
		...
		while($line = fgets($fp, 2000)) {
			...
			if (count($aValues) == count($aFields)) {
				...
				if (empty($row["id"])) {
					...
					echo "Allready Exists: Geb�ude".$e["gebaeude"]."<br>\n";
				}
				if ($i++ < 10) echo $sql."<br>\n";
			}
		}
	}
	echo "Es wurden $num_import Geb�udedaten angelegt bzw. �berschrieben falls schon vorhanden!<br>\n";
	fclose($fp);
	...
	echo "Es wurden $num_import R�ume importiert!<br>\n";
	fclose($fp);
		...
		while($line = fgets($fp, 2000)) {
			...
			if (count($aValues) == count($aFields)) {
				/*for($k = 0; $k < count($aValues); $k++) {
					...
					if ($i < 50) echo "combine k:$k; aFields[$k]:".$aFields[$k]."; aValues[$k]:".$aValues[$k]." e[".$aFields[$k]."]:".$e[$aFields[$k]]."<br>\n";
					else die();
				}
	...
	echo "Es wurden $num_import Mitarbeiter importiert!<br>\n";
	fclose($fp);
}
?>
</body>
