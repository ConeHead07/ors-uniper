<?php 
//if (date("Y-m-d H:i") > "2016-01-30 13:00") die("Das Umzugsportal ist vorübergehend bis voraussichtlich 13.30 Uhr wegen Wartungsarbeiten gesperrt!");
...
if (is_array($ActiveMenu) 
 ...
 && $ActiveMenu["webfreigabe"] == "Ja") {
	...
	for ($i = 0; $i < count($_ParentItems); $i++) {
 		$M = $_ParentItems[$i];
		$MenuAccess.= "Menü ".$M["srv"]." Rechte:".$M["rechte"]." Gruppen:".$M["gruppen"]." Geschï¿½tzt:".$M["geschuetzt"]."<br>\n";
...
if ($srv_error && empty($show_login)) {
	...
	$body_content.= "<br>\n";
	$body_content.= "Bei Fragen zur Erreichbarkeit der gewünschten Informationen kï¿½nnen Sie uns gerne eine E-Mail schreiben an <a href=\"mailto:$postmaster?subject=Seitenfehler:$srv\">$postmaster</a><br>\n";
} else {
