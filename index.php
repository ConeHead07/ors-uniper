<?php
//if (date("Y-m-d H:i") > "2016-01-30 13:00") die("Das Umzugsportal ist vor�bergehend bis voraussichtlich 13.30 Uhr wegen Wartungsarbeiten gesperrt!");
...
if (is_array($ActiveMenu)
    ...
    for ($i = 0; $i < count($_ParentItems); $i++) {
        $M = $_ParentItems[$i];
        $MenuAccess.= "Men� ".$M["srv"]." Rechte:".$M["rechte"]." Gruppen:".$M["gruppen"]." Gesch�tzt:".$M["geschuetzt"]."<br>\n";
        ...
        if ($_ParentItems[$i]["geschuetzt"] == "Ja") {
            if (empty($user)) {
                $srv_error = true;
                $error.= "Zugang nur f�r Mitglieder!<br>\n";
                $show_login = true;
            } else {
                ...
                switch($_ParentItems[$i]["gruppen"]) {
                    case "admin":
                        if ($user["gruppe"] != "admin") {
                            $error.= "Unzul�ssiger Seitenaufruf!<br>\n";
                            $srv_error = true;
                        }
                        break;
...
if ($srv_error && empty($show_login)) {
    ...
    $body_content.= "Falls Sie die Adresse von Hand eingetippt haben, pr�fen Sie bitte die korrekte Schreibweise!\n";
    $body_content.= "M�glicherweise befindet sich die Seite derzeit in Bearbeitung und wurde gesperrt!<br>\n";
    $body_content.= "<br>\n";
    $body_content.= "Bei Fragen zur Erreichbarkeit der gew�nschten Informationen k�nnen Sie uns gerne eine E-Mail schreiben an <a href=\"mailto:$postmaster?subject=Seitenfehler:$srv\">$postmaster</a><br>\n";
} else {
    ...
    if ($ActiveMenu["script"]) {
        if (file_exists($ActiveMenu["script"])) {
            ...
            $error.= "Das Ausgabemodul f�r die Seite \"".$ActiveMenu["name"]."\" konnte nicht geladen werden!<br>\n";
        }
    }
}
