	...
	*, body *, html * { font-family:Arial,Helvetica,sans-serif; font-size:12px; }
	...
	function get_SearchInputListeGebaeude(obj) {
		if (!O('ortsFilter').value) {
			alert("W�hlen Sie erst einenStandort aus!");
			O("ortsFilter").focus();
	...
	function get_SearchInputListeEtage(obj) {
		if (!document.frmMAFilter.elements['ort'].value) {
			alert("W�hlen Sie erst einen Standort aus!");
			//O("ortsFilter").focus();
			//get_SearchInputListeOrte();
			return false;
		}
		if (!document.frmMAFilter.elements['gebaeude'].value) {
			alert("W�hlen Sie erst ein Gebaeude aus!");
			//O("gebaeudeFilter").focus();
			//get_SearchInputListeGebaeude();
			return false;
		}
	...
	function dropMa(MaId, rowId) {
		if (!confirm("Arbeitsplatzdaten l�schen?")) return false;
...
if (count($rows)) {
	...
	$theadColTitles = "<tr><td>#</td><td>Anmkg</td><td>L�schen</td><td>Name</td><td>Vorname</td><td>AP-Nutzung</td><td>Firma (wenn ext.)</td><td>A-Nr.</td><td>GF</td><td>Bereich</td><td>Abtlg</td><td>E-Helfer</td><td>R�umgs-BA</td></tr>\n";
	if (!empty($rows)) for ($i = 0; $i < count($rows); $i++) {
		...
		$liste.= "<tr class=\"".($i%2?'wz2':'wz1')." rowFirstInGroup\" id=\"{$rowId}i1\">\n";
			...
			$liste.= "<td class=\"cellInput\"><div onclick=\"dropMa('".$e["id"]."', '{$rowId}'); \" class=\"jLink\">L�schen</div></td>\n";
...
if (!empty($thisRaum)) {
	...
	echo $raumTypOptions;
	echo "</select>";
	echo "<input type=\"submit\" name=\"editRaumtyp\" value=\"Raumtyp �ndern\" style=\"width:150px;font-size:11px;\">\n";
	echo "<input type=\"hidden\" name=\"raum\" value=\"".fb_htmlEntities($thisRaum["id"])."\">\n";
	echo "<input type=\"hidden\" name=\"etage\" value=\"".fb_htmlEntities($thisRaum["etage"])."\">\n";
	echo "<input type=\"hidden\" name=\"gebaeude\" value=\"".fb_htmlEntities($thisRaum["gebaeude"])."\">\n";
	echo "<input type=\"hidden\" name=\"ort\" value=\"".fb_htmlEntities($thisRaum["ort"])."\">\n";
	...
	echo "<input type=\"submit\" name=\"editRaumnr\" value=\"Raumnr �ndern\" style=\"width:150px;font-size:11px;\">\n";
	echo "<input type=\"hidden\" name=\"raum\" value=\"".fb_htmlEntities($thisRaum["id"])."\">\n";
	echo "<input type=\"hidden\" name=\"etage\" value=\"".fb_htmlEntities($thisRaum["etage"])."\">\n";
	echo "<input type=\"hidden\" name=\"gebaeude\" value=\"".fb_htmlEntities($thisRaum["gebaeude"])."\">\n";
	echo "<input type=\"hidden\" name=\"ort\" value=\"".fb_htmlEntities($thisRaum["ort"])."\">\n";
	...
	echo "<input type=\"submit\" name=\"editRaumflaeche\" value=\"Raumfl�che �ndern\" style=\"width:150px;font-size:11px;\">\n";
	echo "<input type=\"hidden\" name=\"raum\" value=\"".fb_htmlEntities($thisRaum["id"])."\">\n";
	echo "<input type=\"hidden\" name=\"etage\" value=\"".fb_htmlEntities($thisRaum["etage"])."\">\n";
	echo "<input type=\"hidden\" name=\"gebaeude\" value=\"".fb_htmlEntities($thisRaum["gebaeude"])."\">\n";
	echo "<input type=\"hidden\" name=\"ort\" value=\"".fb_htmlEntities($thisRaum["ort"])."\">\n";
	...
	echo "<div><form method=\"get\" action=\"".basename($_SERVER["PHP_SELF"])."\" style=\"display:inline;margin:0;\">\n";
	echo "<input type=\"submit\" name=\"deleteRaum\" value=\"Raum l�schen\" style=\"width:350px;font-size:11px;color:#f00;\">\n";
	echo "<input type=\"hidden\" name=\"raum\" value=\"".fb_htmlEntities($thisRaum["id"])."\">\n";
	echo "<input type=\"hidden\" name=\"etage\" value=\"".fb_htmlEntities($thisRaum["etage"])."\">\n";
	echo "<input type=\"hidden\" name=\"gebaeude\" value=\"".fb_htmlEntities($thisRaum["gebaeude"])."\">\n";
	echo "<input type=\"hidden\" name=\"ort\" value=\"".fb_htmlEntities($thisRaum["ort"])."\">\n";
