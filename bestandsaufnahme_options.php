...
class abteilung {
	...
	function check($e) {
		if (empty($e["bereich"])) $this->error.= "W�hlen Sie einen Bereich (Hauptabteilung) aus!<br>\n";
		if (empty($e["abteilungsname"]) && empty($e["abteilung"]) && empty($e["abteilungsleiter"])) {
			$this->error.= "Geben Sie f�r die Abteilung eine Abk&uuml;rzung, einen Namen od. einen Leiter an!<br>\n";
		}
	...
	// 	Abteilung:
	// - Gesch�ftsbereich (Abk)
	// - Hauptabteilung (Abk)
	// - Abteilungsname (Klartext z.B. Grafik)
	// - Abteilungsleiter
	// Keine Pflichtfelder bei Bestandaufnahme
	// Flag: Was wurde angelegt: Abteilung, Hauptabteilung, Gesch�ftsbereich
}
...
switch($cat) {
	...
	<label for="raumnr">Verrechenbare Fl�che</label>'.get_InputText("verrechenbare_flaeche", $e["verrechenbare_flaeche"]).'<br>
	<input type="hidden" name=cat value="'.fb_htmlEntities($cat).'">
	<input type="hidden" name=ort value="'.fb_htmlEntities($ort).'">
	<input type="hidden" name=gebaeude value="'.fb_htmlEntities($gebaeude).'">
	<input type="hidden" name=etage value="'.fb_htmlEntities($etage).'">
