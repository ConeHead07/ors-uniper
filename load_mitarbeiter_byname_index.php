...
$sql = "SELECT m.id, m.name, m.vorname, m.arbeitsplatznr, m.extern";
$sql.= ", m.extern_firma, m.gf, m.bereich, m.abteilung, m.aufgenommen_am";
$sql.= ", i.gebaeude, i.etage, i.raumnr, i.raum_typ \n";
$sql.= "FROM ".$_TABLE["immobilien"]." i \n";
$sql.= "LEFT JOIN ".$_TABLE["mitarbeiter"]." m ON(m.immobilien_raum_id=i.id)\n";
$sql.= "WHERE m.name LIKE \"".$db->escape($nachname)."\" ";
...
for($i = 0; $i < count($rows); $i++) {
	...
	$JsonMa.= "\t\t\"ma_aa\" : \"".json_escape($e["aufgenommen_am"])."\",\n";
	
	$JsonMa.= "\t\t\"value\" : \"".json_escape($e["name"]." ".$e["vorname"])."\",\n";
	$JsonMa.= "\t\t\"content\" : \"".json_escape($e["name"]." ".$e["vorname"]." &middot;".$e["gebaeude"]." &middot;".$e["etage"]." &middot;".$e["raumnr"]."".($e["arbeitsplatznr"]?"(".$e["arbeitsplatznr"].")":"")." &middot;$abt ".($e["extern_firma"]?" &middot;Fa:".$e["extern_firma"]:"")." &middot;".substr($e["aufgenommen_am"],0,10))."\"\n";
	$JsonMa.= "\t}";
}
$JsonMa.= "]";
