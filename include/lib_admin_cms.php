...
function zzz_parseTag($tname, $tag, &$_CFT) {
	...
	// Filtern g�ltiger Attribut-Eigenschafts-NAMEN
	...
	// Ausfiltern ung�ltiger Attribut-WERTE
	if (isset($_CFT["ATCK"])) {
		reset($_NewTagAttr);
	...
	// Filtern g�ltiger Style-Eigenschafts-NAMEN 
...
function cms_strip_tags($v, $allowableTags = "/default", $_CONF_CTAGS = "") {
	...
	while (strlen($v)) {
		...
		if ($char=="<") {
			...
			// Hier werden ausgeschriebene Internetadressen so pr�pariert,
			// dass der Browser bei langen Adressen automatisch bei Slash selbst
			// den n�tigen Zeilenumbruch vornehmen kann, damit die Seite nicht auseinanderplatzt
			/**/
			if ($tname == "a") {
...
function parse_htmlTag($tag) {
	...
	for ($i=0; $i<strlen($tag); $i++) {
		...
		switch($phrasenOpener) {
			...
			// => '
			// => "
			// => =		Attributwert ist nicht in Anf�hrungszeichen
			...
			switch($z) {
				...
				case " ": // Pr�fe, ob Attribut einzeln im Tag steht => ohne Wertzuweisung
				$temp = substr($tag,$i+1);
				$nextNonBlindChar = substr(trim($temp), 0, 1);
...
// Funktion erledigt diverse Vorarbeiten f�r die Ausgabe
...
function get_cms_items($str,$tagStart,$tagEnd,$setUniqueNeedle) {
	...
	while(strchr($ausgabe,$tagStart) && (strpos($ausgabe,$tagStart)<strrpos($ausgabe,$tagEnd))) {
		...
		// Blockbeginn Buffern der R�ckgabewerte
		...
		// Blockende Buffern der R�ckgabewerte
...
function set_cms_editbar($body_content,$ausgabe) {
	...
	for ($i=0; $i<count($cmsitems_arr); $i++) {
		...
		// Ma�e des Eingabefeldes
		$editBlock=str_replace("/*styleSize*/",$styleSize,$editBlock);
		// Breite der Schaltfl�chenleiste
