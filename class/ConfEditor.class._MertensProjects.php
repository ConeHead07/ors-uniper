...
class ConfEditor {
	...
	function __construct($var_file = "") {
		...
		// Hole Conf-Key
		if ($this->var_file && file_exists($this->var_file)) {
			if (file_exists(str_replace(".inc.php",".fnc.php",$this->var_file))) {
				include_once(str_replace(".inc.php",".fnc.php",$this->var_file));
			}
	...
	function load_struct($aSruct = array()) {
		if (!empty($aStruct)) {
			$this->arrConfProperties = $aStruct;
		}
		if (empty($this->arrConfProperties)) {
	...
	function load_conf($aData = array()) {
		...
		if (empty($this->aCnfData)) {
			// Hole Conf-Werte
			
			if (!isset($_POST[$this->sCnfPostVar])) {
				include($this->var_file);
				$this->aCnfData = $_CONF[$this->sCnfName];	// print_r($this->aCnfData);
			} else {
	...
	function get_form($formAction = "") {
		if (!$formAction) {
			global $_SERVER;
			$formAction = ($_SERVER["QUERY_STRING"]) ? "?".$_SERVER["QUERY_STRING"] : "";
		}
		// Generiere Formular
		return "<form action=$formAction method=post>".$this->get_conf_form($this->arrConfProperties, $this->aCnfData, "")."
	...
	function autorun() {
		...
		if (isset($_POST["SAVE"])) {
			if (!file_exists($this->var_file)) die("#".__LINE__." ".__FILE__." var_file:".$this->var_file." kann nicht geladen werden!<br>\n");
			$this->write_conf();
	...
	function write_conf($ConfKey = "") {
		...
		$CVAR_NAME = "\$_CONF[\"".(!$ConfKey?$this->sCnfName:$ConfKey)."\"]";		
			...
			if (is_array($v)) {
				...
				if ($isStruct && !$isList) {
					...
					if ($isList) {
						...
						$sCnfForm.= "\t<tr><td colspan=2>".$this->get_conf_form($c[$k], $d[$k], $pfad."[$k]")."</td></tr>\n";
					}
				}
			}
		}
		...
		$this->arrConfProperties = array(
			...
			"Fields" => array(
				"{A:Field}" => array(
					...
					"htmlType" => array("select", "str", "text,textarea,radio,checkbox,select single,single multiple,system,hidden"),
...
if (basename(__FILE__) == basename($_SERVER["PHP_SELF"])) {
	...
	echo '</head>'."\n";
	echo '<body>'."\n";
	$CE = new ConfEditor(dirname(__FILE__)."/../include/dummie.inc.php");
	$CE->autorun();
	echo $CE->sCnfForm;
	echo "</body></html>";
}
