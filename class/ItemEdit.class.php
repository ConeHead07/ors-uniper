...
// Idee der 7 Benutzerstufen: Jede n�chste Stufe hat voreingestellt mind. die Rechte der niederen Stufen
...
// Die Benutzerrechte k�nnen auf Datensatz und Feldebene frei �berschrieben werden
...
class ItemListClass 
{
	...
	var $strGroup   = "";
	var $selectFlds = "*";
	var $searchFlds = ""; // default f�r Suche in 'allen Feldern': Wenn leer wie selectFlds
	var $strSQL = "";
	var $strSqlNumAll = "";
		...
		if (isset($this->ItemConf["Joins"])) {
			...
			for($i = 0; $i < count($arrConfNames); $i++) {
				...
				if (isset($this->ItemConf["Joins"][$joinName])) {
					...
					// echo "#".__LINE__." Found ListAutoJoin in _CONF f�r $joinName <br>\n";
					// "listFields"   => "",
					// "listHideKey"  => true,
					// "listHideFlds" => array(),
	...
	function use_join($joinName, $joinListFlds, $blnHideKey, $arrHideFields = array(), $listPosition = "append") {
		...
		if (empty($joinListFlds) || $joinListFlds == "*") {
			if (!is_int(strpos($this->selectFlds, "`".$foreignTblName."`.")) ) {
				...
				echo "#".__LINE__." KONFLIKT: Doppelte Feldauswahl f�r Tabelle `".$foreignTblName."`";
	...
	function get_joinTblAndFld($joinField, &$getArrJoinProps) {
		...
		switch(count($fldDbPath)) {
			...
			if (0 == count($arrListJoins)) {
				echo "#".__LINE__." Unidentifizierbares Join-Feld:$joinField. Es wurden keine Joins f�r die Listenausgabe vorgegeben!<br>\n";
			}
			...
			if (is_array($_CONF) && count($_CONF)) {
				foreach($arrListJoins as $joinName => $arrProps) {
					...
					if (!isset($_CONF[$tblConfKey])) {
						echo "#".__LINE__." Ung�ltiger Conf-Key (_CONF[$tblConfKey]) f�r Tabellen-Join!<br>\n";
					}
			...
			// Es wird gepr�ft, ob Join-Tabelle im Listen-Objekt angemeldet wurde
			// Weitere Pr�fungen, ob Feld tats�chlichh in Tabelle enthalten ist, findet nicht statt
			$jTbl = $fldDbPath[0];
			$jFld = $fldDbPath[1];
			if (!isset($this->arrListJoinsTbl2Name[$jTbl])) {
				...
				echo "#".__LINE__." Unbekannte Join-Tabelle:$jTbl. Es wurden kein Join f�r diese Tabelle registriert oder es liegt keine Conf!<br>\n";
			}
			break;
			
			default:
			// FEHLER: Evtll. wurde noch ein Datenbankname angegeben
			// Ung�ltige Option, da Abfragen nur in der Hauptdatenbank stattfinden !!!
	...
	function set_defaultListButtons() {
		$AttrDel = " class=\"btnDelete\" onclick=\"return confirm('M�chten Sie den Datensatz wirklich l&ouml;schen?')\"";
		...
		if (!empty($operator) || strlen($searchTerm)) {
			...
			if (substr($searchField, -1) != "*") {
				...
				if ($searchArea == $this->ItemClass->arrConf["Title"]) {
					...
					echo "Ung�ltige Feldauswahl: $searchField!<br>\n";
					return false;
				}
	...
	function get_e($fld, $tbl = "") {
		...
		echo "#".__LINE__." Ung�ltiger Index f�r Ergebnis: e[".$this->tmpZahl."] fld:$fld, tbl:$tbl existiert nicht!<br>\n";
		return false;
	}
					...
					foreach($this->arrListButtons as $btnName => $btnVals) {
						...
						if (!empty($btnVals["createByFunction"])) {
							if (function_exists($btnVals["createByFunction"])) {
								...
								echo "#".__LINE__." Error undefined Function '".$btnVals["createByFunction"]."()' f�r ListButton!<br>\n";
							}
						} else {
							$link = "#";
							if ($btnVals['link']) {
					...
					foreach ($this->arrListInput as $el) {
						...
						if (!empty($el["createByFunction"])) {
							if (function_exists($el["createByFunction"])) {
								...
								echo "#".__LINE__." Error undefined Function '".$el["createByFunction"]."()' f�r ListInput!<br>\n";
							}
						} else {
							$rplEl = array();
					...
					for ($t = 0; $t < count($arrShowFieldsById); $t++) {
						...
						if (isset($this->arrColumnHandler[$fld])) {
							if(function_exists($this->arrColumnHandler[$fld])) {
								...
								echo "#".__LINE__." ".__FILE__." Funktion ".$this->arrColumnHandler[$fld]." f�r Feld $fld nicht gefunden!<br>\n";
							}
						}
						if ($SetRawDbVal) {
							$tdClass = $fld;
	...
	*/
	...
	function loadConfByArray(&$conf) {
		...
		if (!is_array($conf) || !isset($conf["Fields"])) {
                    $this->Error.= "#".__LINE__." Ung�ltige oder leere Konf-Daten!<br>\n";
                    return false;
                }
	...
	function checkIfExists() {
		...
		} else $this->Error.= "Fehlende ID:".$this->id." f�r Datenabfrage!<br>\n";
		return false;
	}
	...
	function dbdataToInput($autoload = true) {
            ...
            if (count($this->arrDbdata) || $this->loadDbdata()) {
                if (is_array($this->arrConf["Fields"]) && count($this->arrConf["Fields"]) ) {
                    ...
                    else { $this->Error.= "Es konnten keine DB-Daten �bernommen werden!<br>\n"; }
	...
	// �berschreibbare Werte. 
	// Mit setValue gesetzte Werte d�rfen erst nach 
	// -loadInput()
	// -dbdataToInput()
	// gesetzt, da sie sonst wieder �berschrieben werden
	...
	// Fest voreingestellte Werte, die durch Posts nicht �berschrieben werden k�nnen,
		...
		foreach($this->arrConf["Fields"] as $fN => $fC) {
			if (!isset($this->arrInput[$fN])) {
				switch($this->editMode) { //
					...
					default:
					$this->SysError.= "#".__LINE__." ".basename(__FILE__)." f�r Daten ".$this->arrConf["Title"]."!<br>\n";
	...
	function openTplForm($path) {
		...
		} else $this->Error.= "Es wurde keine Template Datei zum �ffnen angegeben!<br>\n";
	}
						...
						if (!is_null($val) && !empty($val)) {
							if (!is_array($val) && $fC["sysType"] != "set") {
								...
								for ($i = 0; $i < count($aSetVal); $i++) {
									...
									//check_standortverwaltung="D�sseldorf"
									//check_standortverwaltung="D�sseldorf"
								}
							}
						} else {
	...
	function userHasFldAccess($field, $editMode) {
		...
		if (empty($rC[$accessKey]) || $rC[$accessKey] == 0) return true;  // Wenn Zugriff f�r Jeder (gleich: 0) return true
	...
	function checkFieldInput($field, $value, &$error) {
		...
		if ( !$this->userHasFldAccess($fN, $this->editMode) ) {
			if ($this->editMode == "Insert") {
				$error = "Sie haben nicht die erforderlichen Insert-Rechte f�r ".$fC['label']."!<br>\n";
			} else {
				$error = "Sie haben nicht die erforderlichen Update-Rechte f�r ".$fC['label']."!<br>\n";
			}
			return false;
		}

		// Pr�fe, ob ein bzw. kein leerer Wert �bergeben wurde
		if ($fC["editByRuntime"]) {
			$error = "Der Wert f�r " . $fN . " darf nur vom System gesetzt werden!";
			return false;
		}
		if (preg_match("/created|modified/", $fC["sysType"])) {
			$error = "Werte f�r Erstellungs- oder Modifizierungsdatum d�rfen nur vom System gesetzt werden!";
			return false;
		}
		if (preg_match("/PRI/", $fC["key"])) {
			$error = "Primary-Key d�rfen aus Gr�nden der Daten-Integrit�t nur vom System gesetzt werden!";
			return false;
		}
		if ($fC["sysType"] == "password") {
			if (isset($this->arrInput[$fN."_wh"]) && $this->arrInput[$fN."_wh"] != $value) {
				$error = "Passwort stimmt nicht mit Wiederholung �berein!<br>\n";
				return false;
			}
		}
		$isValid = true;
		if ($this->editMode == "Insert" && isset($fC["createByFunction"]) && $fC["createByFunction"] !== "") {
			$error = 'Wert f�r ' . $fN . ' wird bei Erstellung des Eintrags vom System gesetzt!';
			return false;
		}
		...
		if ($fC["checkByFunction"] !== "") {
			if (function_exists($fC["checkByFunction"])) {
				...
				$error = basename(__FILE__)." Fehlende userdefined-Pr�ffunktion in ".$this->arrConf["Title"].": ".$fC["checkByFunction"]." f�r Feld $fN!<br>\n";
				return false;
			}
		} elseif (empty($fC["createByFunction"])
				&& strlen(trim($value))>0 ) {
			// Pr�fe Eingabe auf Typ und L�nge
			$err = "";
			...
			if ($fC["unique"]) {
				...
				if (!$isUnique) {
					$error = "Es existiert bereits ein Eintrag f�r: ".$value."!\n";
					return false;
				}
			}
		} else {
			// Pr�fe, ob Eingabe erforderlich
			if ($fC["required"]) {
				$error = "Leere Angabe!";
				return false;
			}
	...
	function checkInput() {
		...
		if (count($this->arrInput)) {
			foreach($this->arrConf["Fields"] as $fN => $fC) { // f:fieldName, c:fieldConf
				...
				if ( !$this->userHasFldAccess($fN, $this->editMode) ) {
					if ($this->editMode == "Insert") {
						if ($fC["required"]) $this->arrInput[$fN] = $fC["default"];
							...
							if (isset($this->arrInput[$fN]) )
								$this->Warning.= "Sie haben nicht die erforderlichen Insert-Rechte f�r ".$fC['label']."!<br>\n";
							continue;
						}
					} else {
						if (isset($this->arrInput[$fN]) )
							$this->Warning.= "Sie haben nicht die erforderlichen Update-Rechte f�r ".$fC['label']."!<br>\n";
						continue;
					}
				}
				...
				if ($fC["sysType"] == "password") {
					if (isset($this->arrInput[$fN."_wh"]) && $this->arrInput[$fN."_wh"] != $this->arrInput[$fN]) {
						$err = "Passwort stimmt nicht mit Wiederholung �berein!<br>\n";
						$this->Error.= $err;
				...
				if ($fC["checkByFunction"] !== "") {
					if (function_exists($fC["checkByFunction"])) {
						...
						$this->SysError.= basename(__FILE__)." Fehlende userdefined-Pr�ffunktion in ".$this->arrConf["Title"].": ".$fC["checkByFunction"]." f�r Feld $fN!<br>\n";
					...
					if ($isValid && $fC["unique"]) {
						...
						if (!$isValid) $err = "Es existiert bereits ein Eintrag f�r: ".$this->arrInput[$fN]."!\n";
					}
			...
			$this->Error.= "#".__LINE__." Eingabedaten wurden noch nicht �bergeben / geladen!<br>";
			$this->Error.= "F�gen Sie die Eingabedaten mit object->inputLoad(Array Eingaben)<br>!";
			return false;
		}
	}
	...
	function saveInput() {
		...
		if (!$this->Error) {
			...
			foreach($this->arrConf["Fields"] as $fN => $fC) {
				...
				if (!$this->userHasFldAccess($fN, $this->editMode)) {
					if ($this->editMode == "Insert" && $fC["required"]) {
						$this->Error.= "#".__LINE__." Es ist ein Konflikt in der Konfiguratation des Feldes $fN aufgetreten, da Sie keine Eingaberechte f�r eine erforderliche Angabe haben!<br>\n";
						return false;
					}
					continue;
				}
			...
			$this->Error.= "Datensatz kann nicht gespeichert, wenn bei der Pr�fung Fehler aufgetreten sind!<br>\n";
		}
...
<input LINE=\"".__LINE__."\" class=\"ichck\" type=\"checkbox\" name=\"gotoNext\" value=\"New\" chck_gotoNext=\"New\"> ".$this->Labels["gotoNextInsert"];
	...
	function set_autoFormBuilder($mode) {
		...
		switch($mode) {
			...
			default;
			$this->Error.= "Error In Line #".__LINE__." ".basename(__FILE__).": Unbekannte FormBuilder-Wert '$mode', zul�ssig sind 'file','html','render'!<br>\n";
			return false;
		}
	}
	...
	function autoBuildForm($editCmd) {
		...
		switch($this->autoFormBuilder) {
			...
			default:
			$this->Error.= "#".__LINE__." Ung�ltiger Wert f�r autoFormBuilder '".$this->autoFormBuilder."!<br>\n";
			return false;
		}
		return true;
	}
