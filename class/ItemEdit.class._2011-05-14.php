...
class ItemListClass 
{
	...
	function ItemListClass(&$ItemEdit, $selectFlds = "", $setAutoJoin = false, $useListConfId = "default") {
		global $_CONF;
		global $_SERVER;
	...
	function use_join($joinName, $joinListFlds, $blnHideKey, $arrHideFields = array(), $listPosition = "append") {
		// echo "#".__LINE__." f:".__FUNCTION__."(\"$joinName\", \"$joinListFlds\", ".($blnHideKey ? 'true':'false').", ".print_r($arrHideFields, true).", \"$listPosition\")<br>\n";
		...
		for ($i = 0; $i < count($arrHideFields); $i++) {
			if (!in_array("`".$objJoin["foreignTbl"]."`.`".$arrHideFields[$i]."`", $this->arrHideFields)) {
				$this->arrHideFields[] = "`".$objJoin["foreignTbl"]."`.`".$arrHideFields[$i]."`";
			}
		}
		...
		$this->strJoin.= " = `".$objJoin["foreignTbl"]."`.`".$objJoin["foreignKey"]."` \n";
		
		$keyIsSelected = false;
		$tmp2 = "";
		if (empty($joinListFlds) || $joinListFlds == "*") {
			if (!is_int(strpos($this->selectFlds, "`".$objJoin["foreignTbl"]."`.")) ) {
				$tmp2 = "`".$objJoin["foreignTbl"]."`.* ";
				$keyIsSelected = true;
			} else {
				echo "#".__LINE__." KONFLIKT: Doppelte Feldauswahl f�r Tabelle `".$objJoin["foreignTbl"]."`";
			...
			for($i = 0; $i < count($tmp1); $i++) {
				...
				if (!is_int(strpos($this->selectFlds, "`".$objJoin["foreignTbl"]."`.`".$tmp1[$i]."`")) ) {
					if (!is_int(strpos($tmp1[$i], "`")) && !is_int(strpos($tmp1[$i], "(")) ) {
						$tmp2.= ($i ? ", " : "")."`".$objJoin["foreignTbl"]."`.`".$tmp1[$i]."`";
					} else {
						$tmp2.= ($i ? ", " : "").$tmp1[$i];
					}
					if ($tmp[$i] == $objJoin["key"]) $keyIsSelected = true;
				}
			}
			if (!$keyIsSelected) $this->selectFlds.= ", `".$objJoin["foreignTbl"]."`.`".$objJoin["foreignKey"]."`";
		}
	...
	function get_joinTblAndFld($joinField, &$getArrJoinProps) {
		...
		switch(count($fldDbPath)) {
			...
			// Weitere Pr�fungen, ob Feld tats�chlich in Tabelle enthalten ist, findet nicht statt
			$jTbl = $fldDbPath[0];
			$jFld = $fldDbPath[1];
	...
	function set_defaultListButtons() {
		$AttrDel = " class=\"btnDelete\" onclick=\"return confirm('M�chten Sie den Datensatz wirklich l�schen?')\"";
		$this->addButton("open", "", "Bearbeiten", "&view=edit&{trackVars}", "");
		$this->addButton("kill", "", "L�schen", "&view=del&{trackVars}", "", $AttrDel);
	}
	...
	function get_searchForm($defField = "*", $defTerm = "", $arrHidden = array()) {
		...
		$frmSearch.= "<input LINE=\"".__LINE__."\" type=\"image\" src=\"http://www.mertens.ag/images/lupe.gif\" alt=\"Suche starten\" align=\"absmiddle\">"; 
	...
	function mkList($offset, $size, $orderFld, $orderDir) {
		...
		if ($r) {
			...
			if ($this->num) {
				...
				$nr = 0;
				while($this->e = MyDB::fetch_array($r, MYSQL_BOTH)) {
					$nr++;
...
class ItemEdit
{
	...
	function ItemEdit(&$conf, $connid, $user, $id = false) 
	{
	...
	*/
	...
	function loadConfByArray(&$conf) {
		...
		if (is_array($conf) && isset($conf["Fields"])) {
			...
			return true;
		} else { $this->Error.= "#".__LINE__." Ung�ltige oder leere Konf-Daten!<br>\n"; }
		return false;
	}
		...
		foreach($this->arrConf["Fields"] as $fN => $fC) {
			...
			if ($fldAccess) {
				...
				switch($fC["sysType"]) {
					...
					$val_wh = (isset($this->arrInput[$fN_wh])) ? $this->arrInput[$fN_wh] : $this->arrInput[$fN];
					switch($this->editCmd) {
						case "Correct":
						case "Preview":
				...
				if (is_null($val) || $val === "") {
					...
					$this->tplForm = str_replace( $needle, fb_htmlEntities(explode(",", $val)), $this->tplForm);
				}
		...
		if (count($this->arrInput)) {
			foreach($this->arrConf["Fields"] as $fN => $fC) { // f:fieldName, c:fieldConf
				...
				if ($fC["checkByFunction"] !== "") {
					...
					// Pr�fe Eingabe auf Typ und Länge
					$err = "";
		...
		if (!$this->Error) {
			...
			foreach($this->arrConf["Fields"] as $fN => $fC) {
				...
				switch($fC["sysType"]) {
					...
					case "float":
					case "decimal":
					if (!empty($this->arrInput[$fN]) && strlen($this->arrInput[$fN])) {
						if ($SQL_SET) $SQL_SET.= ",\n";
					...
					case "set":
					$setVal = (is_array($this->arrInput[$fN])) ? implode(",", $this->arrInput[$fN]) : $val;
					if ($setVal !== "") {
						if ($SQL_SET) $SQL_SET.= ",\n";
	...
	function isUnique(&$val, $connid, $db, $tbl, &$fld, $key, $id = "") {
		...
		if ($r) {
			list($count) = MyDB::fetch_array($r, MYSQL_NUM);
			$this->db_free_result($r);
			return ($count == 0);
		} else {
	...
	function db_query($SQL, $line = "?") 
	{
		...
		if ($r) {
			...
			$this->db_error = "#$line \$this->dbConnId:".$this->dbConnId.", r:$r, Err:".MyDB::error()."\nQUERY:".fb_htmlEntities($SQL)."\n";
			// die($this->db_error);
		}
	}
			...
			switch($fieldView) {
				...
				if (!$tplInput) {
					if ($v["htmlType"] != "hidden") {
						...
						if (isset($this->arrErrFlds[$k])) $str.= "<em>".trim(array_pop(explode(":",$this->arrErrFlds[$k])))."</em><br>\n";;
...
<input LINE=\"".__LINE__."\" class=\"ichck\" type=\"checkbox\" name=\"gotoNext\" value=\"New\" chck_gotoNext=\"New\"> ".$this->Labels["gotoNextInsert"];
		...
		switch($editCmd) {
			...
			if ($this->id) {
				if ($this->itemExists) {
					if ($this->killItem()) {
						$this->autorun_status = 1;
						$this->Msg.= "Datensatz wurde gel�scht!<br>\n";
					}
				} else $this->Error.= "Zu l�schender Datensatz existiert nicht (mehr)!<br>\n";
			}
			break;
		}
	...
	function autorun_itemlist($aLP = array()) {
		...
		if( class_exists("ItemListClass")) {
			...
			if (!$this->objItemList) {
				$this->objItemList = new ItemListClass($this, $aLP["selectFlds"], null, $aLP["listid"]);
			}
			...
			if( class_exists("listbrowser")) {
				...
				$this->itemList.= "<div class=\"lbnavBox\" style=\"float:left;margin-right:50px;color:#fff;\">".$rlist_nav->get_nav("all")."</div>\n";
				$this->itemList.= "<div class=\"lbsearchBox\" style=\"float:left;color:#fff;\">".$this->searchForm."</div>\n";
