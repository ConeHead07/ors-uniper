...
function ort_getSelection(parentbox) {
	...
	if (typeof(SubItem)=="object") {
		...
		if (document.getElementsByName("eingabe[gebaeude]")) {
			document.getElementsByName("eingabe[gebaeude]")[0].value = "";
			get_fieldindex_gebaeude(document.getElementsByName("eingabe[gebaeude]")[0]);
		}
		return false;
	}
	return true
...
function get_fieldindex_ort(obj) {
	...
	SBConfXF["InputSrc"] = "content";
	
	var SBBoxId = "SBItems";
	var SBBox = getcreateDivBoxById(SBBoxId, "SelBoxEasy SelBoxRegisterOrte");
...
function request_fieldindex_ort(SBBoxId) {
	...
	AjaxRequestUrl = "load_orte_index_registrieren.php?";
...
function get_fieldindex_gebaeude(obj) {
	...
	var SBBoxId = "SBItems";
	var SBBox = getcreateDivBoxById(SBBoxId, "SelBoxEasy SelBoxRegisterGebaeude");
	//alert(typeof(SBBox)+" "+SBBox);
	if (typeof(SBBox)=="object" && typeof(SBBox.SBConf)=="object" && SBBox.SBConf["InputField"]==obj && SBBox.captureEvents) return true;
	var Placeholder = new Array();
	if (typeof(SBBox.SBConf)=="object" && SBBox.captureEvents) SelBox_release(SBBox);
...
function request_fieldindex_gebaeude(SBBoxId, ortePreselect) {
	igWShowLoadingBar(1, "Auswahl wird geladen!", SBBoxId);
	AjaxRequestUrl = "load_gebaeude_index_registrieren.php?";
