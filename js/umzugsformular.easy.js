...
function umzugsantrag_token() {
	if (document.forms["frmUmzugsantrag"].elements["AS[token]"]) {
		return document.forms["frmUmzugsantrag"].elements["AS[token]"].value;
	}
	return false;
}
...
function umzugsantrag_errors(AntragSenden) {
	...
	var ma_err="";
        ...
        $(":input[required]", document.forms["frmUmzugsantrag"]).each(function(idx){
            if ( $.trim( $(this).val() ) !== "") return;
            
            var lbl = $("label[for="+$(this).attr("id")+"]");            
            if ($(this).attr("name") == "AS[umzugstermin]") {
                error+= "Fehlende Eingabe: " + (lbl.attr("title") || lbl.text()).replace("*","") + "<br>\n";
            } else {
                as_err+= "- " + (lbl.attr("title") || lbl.text()).replace("*","") + "\n";
            }            
        });
	...
	if (namedArray["MA[name]"]) {
		...
		for (var i = 0; i < namedArray["MA[name]"].length; i++) {
			...
			else if (InputFields["zapnr"].value=="" && InputFields["zraumnr"].RaumTyp 
				&& InputFields["zraumnr"].RaumTyp=="GBUE") ma_err+= (ma_err?", ":"")+"Ziel-Arbeitsplatznr";
...
function umzugsantrag_reload_attachments(internal) {
    var obj = $("#attachments" + (internal ? "_internal" : "") + "_list");
    if (obj.length) obj.load( obj.attr("data-url"));
}
...
function umzugsantrag_reload(id) {
	...
	var aid = (!arguments.length) ? umzugsantrag_id() : id;
        ...
        if (self.location.href.match(/\bid=[1-9]/)) {
            self.location.reload();
        } else {
            var url = self.location.href + (self.location.href.indexOf("?")==-1 ? "?" : "&") + "id="+aid;
...
function umzugsantrag_add_internal_attachement() {
	var aid = umzugsantrag_id();
	var token = umzugsantrag_token();
	if (typeof(winUp)=="object" && typeof(winUp.closed)!="undefined" && !winUp.closed) winUp.close();
...
function getUmzugsItemTag(obj, tagName, tagClass) {
	if (typeof(O(obj))!="object") return false;
	var myNode = obj;
	...
	var aElements = parentTbl.getElementsByTagName(tagName);
	
	for(var i = 0; i < aElements.length; i++) {
		if (aElements[i].className.indexOf(tagClass)!=-1) 
			return aElements[i];
	}
	return null;
}
...
function standort_ort_getSelection(parentbox) {
	...
	if (typeof(SubItem)=="object") {
		...
		if (typeof(InputFields["AS[gebaeude]"])=="object") {
			...
			if (O("ASGebaeudeUsrInput")) O("ASGebaeudeUsrInput").click(); // get_standort_gebaeude( O("ASGebaeudeUsrInput") );
		}
		return false;
	}
}
...
function standort_gebaeude_getSelection(parentbox) {
	...
	SubItem = SelBox_getSelectedItem(parentbox);
        ...
        if ( typeof( O(parentbox.SBConf["ValueTarget"])) === "object" ) {
            O(parentbox.SBConf["ValueTarget"]).value = SubItem.value;
            parentbox.SBConf["InputField"].value = SubItem.content;
            // alert( "#710 umzugsformular.easy.js SubItem: " + JSON.stringify(SubItem));
        } else if (typeof(SubItem)=="object") {
		parentbox.SBConf["InputField"].value = SubItem.value;
	}
        
        if (typeof(InputFields["AS[etage]"])=="object") {
            InputFields["AS[etage]"].value = "";
            if (O("ASEtageUsrInput")) O("ASEtageUsrInput").click(); // get_standort_gebaeude( O("ASGebaeudeUsrInput") );
        }
        return true;
}

function get_standort_gebaeude(obj, valueTarget) {
	if (!obj || typeof(O(obj))!="object") return false;
	obj = O(obj);
	obj.hasFocus = true;
	...
	var SBConfGebaeude = getCopyOfArray(SBConfDefault);
	SBConfGebaeude["InputField"] = obj;
	if (valueTarget) SBConfGebaeude["ValueTarget"] = valueTarget;
...
function gebaeude_etage_getSelection(parentbox) {
	if (!parentbox || typeof(O(parentbox))!="object") return false;
	var InputFields = getUmzugsItemInputFields(parentbox.SBConf["InputField"]);
...
function get_gebaeude_etage(obj, valueTarget) {
	if (!obj || typeof(O(obj))!="object") return false;
	obj = O(obj);
	obj.hasFocus = true;
	var InputFields = getUmzugsItemInputFields(obj);
	var o = (InputFields && InputFields["AS[gebaeude]"])?InputFields["AS[gebaeude]"].value:"";
	var optionsEtagen;
	...
	if (o && load_optionsEtagenlisteByGebaeude(o)) {
		optionsEtagen = optionsEtagenlisteByGebaeude;
	} else if (load_optionsAllegebaeude()) {
            alert("Etagenauswahl konnte nicht geladen werden");
            return;
	}
	
	var SBConfEtage = getCopyOfArray(SBConfDefault);
	SBConfEtage["InputField"] = obj;
	if (valueTarget) SBConfEtage["ValueTarget"] = valueTarget;
	SBConfEtage["OnSelect"] = gebaeude_etage_getSelection;
	
	var SBBox = getcreateDivBoxById("SBItems", "SelBoxEasy SelBoxOrte");
	
	if (typeof(SBBox.SBConf)!="object" || SBBox.SBConf["InputField"]!=obj || !SBBox.captureEvents) {
		if (typeof(SBBox.SBConf)=="object") SelBox_release(SBBox);
		SelBox_capture(SBBox, SBConfEtage, optionsEtagen);
		dockBox(obj, SBBox);
	}
	return true;
}
...
function gebaeude_getSelection(parentbox) {
	...
	if (SubItem) {
		...
		switch(parentbox.SBConf["InputType"]) {
			...
			case "Gebaeude":
			parentbox.SBConf["InputField"].value = (parentbox.SBConf["InputSrc"] !== "content" ? SubItem.value : SubItem.content);
			parentbox.SBConf["InputField"].finalValue = SubItem.value;
...
function get_gebaeude(obj, valueTarget) {
        if (typeof(O(obj))!="object") return false;
	obj.hasFocus = true;
	obj = O(obj);
	...
	SBConfGebaeude["InputField"] = obj;
	SBConfGebaeude["InputSrc"] = "content";
	SBConfGebaeude["ValueTarget"] = valueTarget;
...
function raumnr_getSelection(parentbox) {
    ...
    var z = (obj.name.indexOf("MA[z")==0)?"z":"";
		...
		if (SubItem.RaumTyp == "GBUE") {
			...
			InputFields[z+"apnr"].readOnly = false;
		} else {
			InputFields[z+"apnr"].value = "";
			InputFields[z+"apnr"].readOnly = true;
		}
		return false;
	} else {
		parentbox.SBConf["InputField"].value = "";
...
function mapflege_getSelection(parentbox) {
	...
	switch(parentbox.SBConf["InputType"]) {
		...
		if (SubItem.ma_aa) {
			var InfoTag = getUmzugsItemTag(InputFields["zabteilung"], "td", "zspace");
			if (InfoTag) InfoTag.innerHTML = SubItem.ma_aa;
		}
...
function mapflege_save() {
	...
	umzugstermin+= "-"+(dt.getMonth()<9?"0"+(dt.getMonth()+1).toString():dt.getMonth()+1);
...
function load_optionsAllegebaeude() {
	...
	var j=0;
	for(i in Gebaeude) {
		optionsAlleGebaeude[j++] = {value:i, content:Gebaeude[i]["Adresse"]};
	}
	return true;
}
...
function load_optionsGebaeudeByOrt(ort) {
	...
	var j=0;
	for(i in Orte[ort]) {
		optionsGebaeudeByOrt[j++] = {value:i, content:Gebaeude[i]["Adresse"]};
	}
	return true;
}
...
function load_optionsEtagenlisteByGebaeude(g) {
	if (typeof(Gebaeude)!="object" || !Gebaeude[g]) return false;
	optionsEtagenlisteByGebaeude = [];
	var i;
	var j=0;
	for(i in Gebaeude[g]["Etagenliste"]) {
		optionsEtagenlisteByGebaeude[j++] = {value:Gebaeude[g]["Etagenliste"][i], content:Gebaeude[g]["Etagenliste"][i]};
	}
	return true;
}
