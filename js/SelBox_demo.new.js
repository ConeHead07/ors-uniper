...
function SelBoxInit(initInputObj, aConf) { // Ursprüngl. Params statt aConf: initLSName, initSBCId, initIsLiveSearch, initMultiple) {
	...
	if (refreshObj) {
		...
		if (obj.SelBox.Conf["LSIsLiveSearch"]) obj.onkeyup=obj.SelBox.SelBox_sendQuery;
...
function SelBox_filterItems() {
	for (var i = 0; i < this.Items.childs.length; i++) {
		...
		if (this.inputField.value == "" || this.Items.childs[i].getElementsByTagName("input")[0].value.toLowerCase().indexOf(this.inputField.value.toLowerCase())==0) {
