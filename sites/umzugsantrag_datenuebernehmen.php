...
function umzugsantrag_datenuebernehmen($AID) {
	...
	if ($AID) {
		...
		for($i = 0; $i < count($MAIDs); $i++) {
			...
			$sql.= " `arbeitsplatznr` = ".($MAItems[$i]["zapnr"] ? "\"".$db->escape($MAItems[$i]["zapnr"])."\"":"NULL").",\n";
			...
			if ($raumdaten_alt["raum_typ"] == "GBUE" && $MAItems[$i]["apnr"]) {
				$rows = get_arbeitsplatz_belegung($raumdaten_alt["id"], $MAItems[$i]["apnr"]);
				if (empty($rows) || !count($rows)) {
					create_spare($raumdaten_alt["raum_typ"], $MAItems[$i]["apnr"]);
					if ($db->error()) {
