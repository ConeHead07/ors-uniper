...
if (!$token && $aid) {
    switch($response) {
        case 'json':
            $custom_error['jquery-upload-file-error']="Es wurde kein Token �bergeben";
    ...
    die("<strong>Es wurde kein token &uuml;bergeben!</strong> <br>"
        ...
        ."Einen Token erhalten Sie automatisch beim �ffnen eines Leistungsformulars.<br>"
        ."<br>"
...
if (!empty($drop)) {
	if ($isAdmin) {
		...
		if ($dropError) $uploadError.= ($uploadError?"<br>\n":"").$dropError;
                else $uploadMsg = "Datei wurde gel�scht!";
	} else {
