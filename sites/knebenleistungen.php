...
$sqlWhere= "WHERE createduid=\"".$db->escape($user["uid"])."\" \n";
switch($cat) {
	case "Neu":
...
if (is_array($all)) foreach($all as $i => $item) {
	...
	$Umzuege[$i]["LinkOpen"] = "?s=knebenleistung&id=".$item["id"];
	$Umzuege[$i]["uhrzeit"] = substr($item["uhrzeit"],0,5);
...
$body_content.= $Tpl->fetch("knebenleisungen_liste.html");
