...
function umzugsantrag_antrag_stornieren() {
	...
	if(!$AID) {
		$error.= "Fehlende Antrags-ID f�r Storniervorgang!<br>\n";
		return false;
	}
	...
	$db->query($sql);
	if ($db->error()) {
		$msg.= "Systemehler beim Stornieren des Auftrag! Bitte versuchen Sie es sp�ter noch mal oder wenden Sie sich an die Bearbeitungsstelle!<br>\n";
		$msg.= $db->error()."<br>\n".$sql."<br>\n";
		return false;
	} else {
	...
	if ($db->error()) {
		$msg.= "Fehler beim L�schen der Mitarbeiterdaten!<br>\n";
	}
	return true;
}
